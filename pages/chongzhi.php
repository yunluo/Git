...
if (isset($_POST['pay_form']) && $_POST['pay_form'] == 'send') {
	...
	if($yanchi > 28860){
		if (!empty($rmb_number)) {
			...
			$post_content = '充值RMB金额：' . $rmb_number . '元<br>充值人昵称：' . $current_user->display_name . '<br>充值邮箱：' . $current_user->user_email . '<br>充值金币：' . $point_number . '金币<br>充值码：' . $tokon . '<br>充值人ID：' . $current_user->ID . '<br>如果您确定已支付宝充值到账，<a target="_blank" href="'.admin_url().'admin.php?page=points&action=success&point_id='.$pointid.'">请立即点击更新金币状态</a><br><br>如果您确定未扫码支付，<a target="_blank" href="'.admin_url().'admin.php?page=points&action=delete&point_id='.$pointid.'">请立即点击删除金币记录</a><br><br>如果您确定用户重复提交订单，<a target="_blank" href="'.admin_url().'admin.php?page=points&action=cancel&point_id='.$pointid.'">请立即点击关闭充值订单</a>';
			$headers = "Content-Type:text/html;charset=UTF-8\n";
			wp_mail($tomail, 'Hi,站长，有人充值!!！充值人ID：' . $current_user->ID . '', $post_content, $headers);
