...
if (isset($_POST['pay_form']) && $_POST['pay_form'] == 'send') {
	...
	if($yanchi > 28860){
		if (!empty($rmb_number) && $rmb_number >= '1' && !is_float($rmb_number)) {
			...
			$post_content = '<style type="text/css">a:visited{color:#d90a81}</style><div class="wrapper" style="width:100%;padding-top:16px;padding-bottom:10px;"><br style="clear:both;height:0"><div class="content" style="background: none repeat scroll 0 0 #FFFFFF;border:1px solid #E9E9E9;margin: 2px 0 0; padding: 30px;">尊敬的管理员，您好：<p style="border-top: 1px solid #DDDDDD;margin:15px 0 25px;padding:15px;">充值码：' . $tokon . '<br>充值RMB金额：' . $rmb_number . '元<br>充值人昵称：' . $current_user->display_name . '<br>充值邮箱：' . $current_user->user_email . '<br>充值金币：' . $point_number . '金币<br>如果您确定已充值到账，<a target="_blank" href="'.admin_url().'admin.php?page=points&action=success&point_id='.$pointid.'">请立即点击更新金币状态</a><br><br>如果您确定未扫码支付，<a target="_blank" href="'.admin_url().'admin.php?page=points&action=fail&point_id='.$pointid.'">请立即点击取消金币充值</a><br><br>如果您确定重复订单，<a target="_blank" href="'.admin_url().'admin.php?page=points&action=cancel&point_id='.$pointid.'">请立即点击关闭充值订单</a>
			<br><br>如果您需要修改订单，<a target="_blank" href="'.admin_url().'admin.php?page=points&action=edit&point_id='.$pointid.'">请立即点击修改当前充值</a>
			</p><p class="footer" style="border-top: 1px solid #DDDDDD; padding-top:6px;margin-top:25px;color:#838383;">系统邮件，请勿回复<span style="float:right;"><a href="'.admin_url().'admin.php?page=points" target="_blank">积分管理</a></span></p></div></div>';
			$headers = "Content-Type:text/html;charset=UTF-8\n";
			wp_mail($tomail, '[在线充值]有充值请求，请处理！充值ID：' . $current_user->ID . '', $post_content, $headers);
			...
			echo '<script type="text/javascript">alert("充值失败，请输入充值金额\n请确保充值金额不低于1元");window.location = document.referrer;</script>';
		}
	}else{
...
<form method="post" action="<?php echo $_SERVER["REQUEST_URI"];?>">
	...
	<hr/>
	<p>
	<img style="margin-left:0px;height:auto;width:auto" src="<?php echo git_get_option('git_chongzhi_qr');?>">
	</p>
	<hr/>
