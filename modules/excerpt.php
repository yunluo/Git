...
<?php } else if ( has_post_format( 'gallery' )) { //日志 ?>
    ...
    <a class="gradient-mask post-img img-response" href="<?php the_permalink() ?>"><?php
        if (git_get_option('git_qncdn_b') ) {
            if(git_get_option('git_cdnurl_style') ){
                $githumb7 = '!githumb7.jpg';
            }else{
    ...
    <div class="post-body hidden-xs note">
        <?php
        $excerpt = $post->post_excerpt;
        if (empty($excerpt)) {
            echo deel_strimwidth(strip_tags(apply_filters('the_content', strip_shortcodes($post->post_content))) , 0, 170 , '……<a href="' . get_permalink() . '" rel="nofollow" class="more-link">继续阅读 &raquo;</a>');
        } else {
            echo deel_strimwidth(strip_tags(apply_filters('the_excerpt', strip_shortcodes($post->post_excerpt))) , 0, 170 , '……<a href="' . get_permalink() . '" rel="nofollow" class="more-link">继续阅读 &raquo;</a>');
        } ?>
    </div>
    <ul class="post-meta-bottom">
    <li class="post-meta-author">
